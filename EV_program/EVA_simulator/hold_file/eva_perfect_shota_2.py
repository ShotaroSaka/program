...
class EV(object) :
    ...
    def set_calc_param(self, gamma, kappa_r, base_p, Es, Eb) -> None:
    ...
    def set_price_param(self) -> None:
        if self._kind == "S": 
            self._price = self._Es * self._base_p         # 一台しかいないときに値段（一番良い値段）
            self._price_pre = self._Es * self._base_p     # 混雑状態が維持されたときの価格
            self._price_rate = self._base_p             # 混雑具合で変動する価格レート(1kWhあたり)
...
class EVA(object) :
    ...
    def set_param(self, k1, k2, k3, limit_As, limit_Ar) -> None:
        ...
        self._kappa_price = 1                     # 多項ロジットモデルでの重み（値段）
        #self._kappa_price = int(sys.argv[1])       # 多項ロジットモデルでの重み（値段）
    ...
    def set_init(self, init_x, init_p) -> None:
...
class Simulator(object):
    def __init__(self, stT, endT):
        ...
        self._lam = 10                 # EV の発生頻度
        self._lamda = 0.1
        #self._lamda = float(sys.argv[2])
    ...
    def EVA_choise(self, EV):            # 確率をもとに EVA を選択する
        self.calc_v(EV)
        self.calc_pro(EV)
    ...
    def dep_EV_pro(self, EVA_id, EV_id) -> None:  # 出発するEVの出発処理
    ...
    def ev_price_ave(self) -> None:
        ...
        S_ave = S_sum / i
        B_ave = B_sum / j
        print("S_ave_price: {0} B_ave_price: {1}".format(S_ave, B_ave))
...
def make_EV(i, time, sim, EVA_can):
    ev = EV(id = i)
...
def main():
    ...
    sim.read_csv("Sanda_EVA_candidate.csv")
    #sim.read_csv(sys.argv[0])
    ...
    while (t < endT):
        if (next_arr_time < next_dep_time):
            ...
            EV = make_EV(i = EV_num, time = t, sim = sim,
                         EVA_can = EVA_candidate.__next__())        # EV を作成    
            EV_num += 1

            print("EV_kind: {0} Event arrT: {1} ".format(EV._kind, t))
            
            pickup_EVA_id = sim.EVA_choise(EV)                      # 売買する EVA を選択
            sim._EVA_list[pickup_EVA_id].add_EV(EV)                 # EVA に追加
            ...
            next_dep_EV = sim.next_dep_calc()     # 次に出発する EV を格納
            ...
            next_dep_EV = sim.next_dep_calc()              # 次に出発する EV を計算
            next_dep_time = next_dep_EV._depT              # 次に出発する EV の出発時間

            print_next_event_time(next_arr_time, next_dep_time)
           
        sim.print_EV_price_rate_1()
    sim.print_dep_list()
