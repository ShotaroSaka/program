...
class EVA(object):
    ...
    def add_EV(self, ev) -> None:
        ...
        if self._battery_exists:
            ...
            if self._EV_list[0]._kind == ev._kind:
                self.remove_EVA_battery()
        else:
            # 訪れた EV の電力売買を可能にするため EVA を追加
            if self._S_EV_num == 0:
                self.generate_EVA_battery(ev, kind = "S")
            elif self._B_EV_num == 0:
                self.generate_EVA_battery(ev, kind = "B")
    ...
    def generate_EVA_battery(self, ev, kind):
...
class Simulator(object):
    ...
    def get_next_dep_EV(self) -> object:
        ...
        next_dep_ev.set_param_time(arrT = 0, depT = 100, nowT = 0)
        for eva in self._EVA_list:     
    ...
    def simulation(self):
        ...
        while (t < self._endT):
            ...
            self.calc_all_EVA(t)
            # 次に出発する EV の時刻を格納          
            next_dep_EV = self.get_next_dep_EV()      
            next_depT = next_dep_EV.get_depT()
