...
class Simulator(object):
    ...
    def all_get_id(self):  # 全てのidをGに格納しておく
        for EVA in self._EVA_list:
            ...
            id = ox.get_nearest_node(self.G, point)
            EVA.set_id(id)
    ...
    def get_shortest_path(self, EV):
        ...
        path = ox.shortest_path(self.G, start._place_id, end._place_id)  # 最短パスを求めている
...
def select_place(data):
    pickup_EV_place = random.choices(data, k = 1, weights = data[:][2])
...
def make_EV(sim, t, i, kind, place):
    if kind == "S":
        ...
        seller.set_place(latitude = place[0], longitude = place[1], population = place[2], sim = sim)
        ...
        buyer.set_place(latitude = place[0], longitude = place[1], population = place[2], sim = sim)
...
def main():
    ...
    while (t < endT):
        if (next_arr_time < next_dep_time):
            ...
            EV = make_EV(sim = sim, t = t, num = i, kind = EV_kind, place = EV_place)
            print(sim.pickup_EVA(EV))
