            ...
            for i in range(k*m):
                ...
                if j <= ev._price < j + k:
                    if ev._kind == "S":
                        ...
                        price_rate_b[i] += 1
            #print("kind: {0} price: {1}"
                  #.format(ev._kind, ev._price))

        for i in range(len(price_rate_s)):
            print("S_{0}: {1} {2} num: {3}"
                  .format(i, i*k, i*k + (k-1), price_rate_s[i]))
        for i in range(len(price_rate_b)):
            print("B_{0}: {1} {2} num: {3}"
                  .format(i, i*k, i*k + (k-1), price_rate_b[i]))      
