...
def make_EV(i, sim, kind, t, EVA_candidate):
    if kind == "S":
        ...
        ev.set_calc_param(gamma = alpha, kappa_r = 0.1, base_p = 15, Es = 50, Eb = 0)
        ev.set_EVA_candidate(EVA_candidate)
        
    elif kind == "B":
        ev = EV(id = i, kind = "B")
        ev.set_time(arrT = t, depT = t + 50, t_a = t)
        ev.set_calc_param(gamma = beta, kappa_r = 0.1, base_p = 16, Es = 0, Eb = 50)
        ev.set_EVA_candidate(EVA_candidate)
...
def print_dep_list(dep_list):
    print("dep_EV_list")
    for ev in dep_list:
        print(" EVA {6} id: {0} kind: {1} gamma:{6} arrT: {2}  depT: {3} totalT: {4} price: {5}"
...
def main():
    ...
    while (t < endT):
        if (next_arr_time < next_dep_time):
            ...
            sim._EVA_list[pickup_EVA_id].add_EV(EV)
