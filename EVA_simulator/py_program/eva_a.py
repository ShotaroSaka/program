...
class EV(object) :
    def __init__(self, id):
        ...
        self._total_E_kWh = 0                           # 今まで取引した電力量
        self._total_P_yen = 0                           # 電力売買価格
        
        self._kind = self.set_kind()              # kind of EVs (kind = 'S' or kind = 'B') in the group
    ...
    def set_param_time(self, arrT, depT, t_a) -> None:
        ...
        self._now_h = t_a                    # 現在の時間をもたせる

        
    def set_param_private(self, kappa_dis, kappa_price, kappa_nu):
        self._kappa_dis = kappa_dis          # 多項ロジットモデルでの重み（距離）
        self._kappa_P = kappa_price      # 多項ロジットモデルでの重み（値段）
    ...
    def calc_P_now(self, rate, p, t_h) -> None:
        passedT = t_h - self._now_h
        if self._kind == "S":    # for sellers
            p = self._kappa_s * rate**(self._gamma_fact) * (p[0]-p[2])
            self._total_P_yen += rate*passedT*self._base_P - p*passedT        # 今まで取引した価格    
            self._P_rate = (rate*self._base_P - p) / rate                   # 1 kWh あたりの値段

        elif self._kind == "B":  # for buyers
            p = self._kappa_b * rate**(self._gamma_fact) * (p[1]+p[2])
            self._total_P_yen += rate*passedT*self._base_P + p*passedT
            self._P_rate = (rate*self._base_P + p) / rate 
           
           
    def calc_E_now(self, rate, t_h) -> None:  # 完了した電力量
        self._total_E_kWh += rate*(t_h - self._now_h)
...
class EVA(object) :
    ...
    def calc_dep_time(self, rate) -> None:      # 電力売買が完了する時間を計算
        for ev in self._EV_list.values():
            ev._depT = ev._now_h + (ev._request_E_kWh - ev._total_E_kWh)/rate
    ...
    def get_conv(self, e_time) -> None:
        ...
        i = 0
        for ev in self._EV_list.values():
            ev_i_rate = self._opt.x[i]                           # レート of EV id
            ev._rate = ev_i_rate 

            ev.calc_P_now(ev_i_rate, (p1, p2, p3), e_time)       # 価格を更新する（etimeまでの合計価格）
            ev.calc_E_now(ev_i_rate, e_time)                 # etime までの総充電量
            ev._now_h = e_time                                   # 今の時間を保存する

            self.calc_deptime(ev_i_rate)                         # レートと enow が更新されたので，depT を計算する
...
class Simulator(object):
    def __init__(self, stT, endT):
        ...
        self._dep_ev_list = []          # 出発した EV のリスト
    ...
    def choice_EVA(self, ev):            # 確率をもとに EVA を選択する
        self.calc_v(ev)
        EVA_can_id = [EVA_id for EVA_id in ev.EVA_can]
    ...
    def calc_v(self, ev) -> None:
        ...
        for eva_id, eva_dis in ev.EVA_can.items():
            if ev._kind == "S":   P_rate = -self._EVA_list[eva_id]._EV_list[0]._P_rate
            elif ev._kind == "B": P_rate =  self._EVA_list[eva_id]._EV_list[1]._P_rate

            v_cal = ev._kappa_dis*eva_dis + ev._kappa_price*P_rate   # 多項ロジットモデル
            v_list.append(v_cal)

        all_v = 0
        for i in v_list:
            all_v += math.exp(-ev._nu*i)
            
        self._pr = [math.exp(-ev._nu*j) / all_v for j in v_list]
    ...
    def calc_next_dep(self) -> object:   # 次に出発するEVの時間を計算
        ...
        newt_dep_ev._depT = 100
        for eva in self._EVA_list:     
            for ev in eva._EV_list.values():
                if ev._depT < next_dep_ev._depT:
                    next_dep_ev = ev
    ...
    def process_dep_EV(self, EVA_id, EV_id) -> None:  # 出発するEVの出発処理
        dep_ev = self._EVA_list[EVA_id]._EV_list.pop(EV_id)
        self.dep_ev_list.append(dep_ev)
...
def main():
    ...
    while (t < endT):
        if (next_arr_time < next_dep_time):
            ...
            sim.process_dep_EV(next_dep_EV._EVA_id, next_dep_EV._id)   # 出発処理
            sim.print_EV_num()            
