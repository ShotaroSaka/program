...
class EV(object) :
    def __init__(self, id, kind = None):
        ...
        self._kappa_s = 150                                # 売電価格の計算式における係数
        self._kappa_b = 150                                # 買電価格の計算式における係数
...
class EVA(object) :
    ...
    def set_param(self, k1, k2, k3, limit_As, limit_Ar, kappa) -> None:
        ...
        self._kappa_dis = 0.03            # 多項ロジットモデルでの重み（距離）
...
class Simulator(object):
    ...
    def calc_v(self, EV) -> None:
        ...
        for eva_id, eva_dis in EV.EVA_can.items():
            ...
            v_cal = -kappa_dis*eva_dis + kappa_price*price_rate   # 多項ロジットモデル
    ...
    def calc_max_variable(self):
        self.max_S_num = [0]*len(self._EVA_list)
        self.max_B_num = [0]*len(self._EVA_list)
    ...
    def print_EV_num_1(self, t, a, kind) -> None:
        if kind == "S":
            for eva in self._EVA_list:
                if eva._id == a:
                    print("{0} {1}"
    ...
    def print_EV_num_2(self, t) -> None:
        for eva in self._EVA_list:
            print("EVA: {0} time: {1} S_num: {2} Bnum: {3}"
                  .format(eva._id, t, eva._s_num, eva._b_num))


    def print_EV_num_3(self) -> None:
        i = 0
    ...
    def EV_num_calc(self) -> None:
        for eva in self._EVA_list:
            if self.max_S_num[eva._id] < eva._s_num:
                self.max_S_num[eva._id] = eva._s_num
            if self.max_B_num[eva._id] < eva._b_num:
                self.max_B_num[eva._id] = eva._b_num
...
def main():
    ...
    EV_num = 2                         # 初期の EV の数
    EVA_num = int(sys.argv[3])             # EVA の台数
    price_kappa = float(sys.argv[2])   # 多項ロジットモデルでの価格の重み
    make_EVA(EVA_num, sim, price_kappa)
    
    next_arr_time = sim.rnd_exp()
    next_dep_time = endT

    sim.calc_max_variable()
    while (t < endT):
