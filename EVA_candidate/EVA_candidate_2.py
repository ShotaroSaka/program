...
class Simulate(object):
    ...
    def choice_random_des(self, des_num, start) -> object:
        '''目的地リストから des_num 個目的地候補を選ぶ
           多項ロジットモデルで２つの指標（人気度，距離）を用いて決定'''
        start_point = (start._lat, start._lon)

        # 各目的地の効用を格納する配列
        des_candidate = []
        for des in self._des_list:
            # スタート地点と目的地との直線距離を測定
            des_point = (des._lat, des._lon)
            dis = geodesic(start_point, des_point).km

            # 多項ロジットモデルで効用を計算
            V = self._alpha*des._pop + self._beta*dis
            des_candidate.append(V)
            self.print_des_factor(des._id, des._pop, dis, V)
...
def main():
    ...
    for i in range(n):
        ...
        # 目的地の候補を決定
        end_point_list = sim.choice_random_des(dis_num, start_point)        
